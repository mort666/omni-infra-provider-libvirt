...
func DefaultEntry[T any](data *T) *T {
	if data != nil {
		return data
	} 
...
// NewDefaultConfig returns a minimal default config based on name, user and
// ssh public key
func NewTalosConfig(userdata *UserData, metadata *MetaData, vendordata *string, networkconfig *NetworkConfig, logger *zap.Logger) *Config {
	config := &Config{
		UserData:      DefaultEntry(userdata),
		MetaData:      DefaultEntry(metadata),
		NetworkConfig: DefaultEntry(networkconfig),
		VendorData:    *DefaultEntry(vendordata),
		TalosConfig:   true,
		logger:        logger,
	}
